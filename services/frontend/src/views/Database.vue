...
<div>
  ...
  <list-with-pagination :list-total="7" :list-size="2" :button-panel-size="2">
  ...
  <div v-else>
    <b-button-toolbar>
      <b-button-group>
        <b-btn @click="scrollButtonsToStart()">
          &laquo;
        </b-btn>
      ...
      <b-button-group class="mx-1">
        <b-btn class="mx-1 pagination" v-for="i in numOfPages" v-if="isButtonPageVisible(i-1)" :key="i" @click="scrollToPage(i-1)" :variant="buttonVariant(i-1)">
          {{ buttonName(i-1)}}
        </b-btn>
      </b-button-group>
      <b-button-group>
        ...
        <b-btn @click="scrollButtonsToEnd()">
          &raquo;
        </b-btn>
      </b-button-group>
    </b-button-toolbar>
    <b-list-group>
      <b-list-group-item v-for="(t, index) in tech" :key="t" :active="index===page.activeIndex">
        {{pi2page(page.activePageIndex)+index+1}}: {{t.name}} | {{t.status}}
        <b-badge class="mx-1" v-for="t in t.tags" :key="t">
          {{t}}
        </b-badge>
...
export default {
  ...
  methods: {
    ...
    scrollButtonBar(offset) {
      ...
      if (newFirstVisible >= 0 && newFirstVisible + this.page.visible < this.numOfPages) {
