  ...
  <b-row>
    ...
    <b-col cols="6">
      ...
      <div v-else>
        ...
        <list-with-pagination ref="pagination" :list-size="25" v-bind:list-total="filteredTechComponents.length" v-bind:activeOrdinal="activeOrdinal" :button-panel-size="7">
          <tech-component slot-scope="props" v-if="props.isOn" :tech="filteredTechComponents[props.listSeq]" v-bind:active="filteredTechComponents[props.listSeq].uid === activeId">
          </tech-component>
  ...
  computed: {
    ...
    amounts() {
      ...
      this.$store.getters.lifeCycle.forEach(
        item => am = { ...am,
          [item.name]: this.techComponents.filter(t => t.status === item.name).length
        }
      )
      return am
    },
