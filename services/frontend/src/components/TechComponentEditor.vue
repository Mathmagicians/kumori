            ...
            <b-list-group-item>
              ...
              <b-form-row>
                ...
                <b-form-group
                  ...
                  label-for="lcSelect"
                  description="What is the current status on Kumori?"
                >
                  <b-form-select
                    id="lcSelect"
            ...
            <b-list-group-item>
              ...
              <b-form-row>
                <b-form-group
                  ...
                  :key="index"
                  v-show="
                    isLeftDefined(index) && treeGeneration(index).length > 0
                  "
                  :id="'txLevel_' + level"
                  :label="level"
...
export default {
  ...
  created() {
    ...
    let myTech = this.$store.getters.tech
      .filter(tech => tech.uid === this.uid)
      .pop();
    ...
    this.techModel.tags.forEach(
      tag =>
        (this.txModel[
          this.taxonomyLevels[this.findNodeForName(tag).level - 1]
        ] = tag)
    );
  },
  methods: {
    onSubmit() {
      ...
      this.uid
        ? this.$store.dispatch("editTechComponent", this.techModel)
        : this.$store.dispatch("createTechComponent", this.techModel);
      this.$router.go(-1);
    },
    onReset() {
    ...
    updateTax(index) {
      ...
      this.techModel.tags = Object.values(this.txModel).filter(
        tx => tx && tx !== "" && tx !== undefined
      );
    }
  }
};
