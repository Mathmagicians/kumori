...
<div>
  <b-card no-body class="mb-2">
    <b-input-group>
      <b-form-input title="Type what you want to search for - technology name, use case, anything ..." id="searchInput" type="text" v-model="searchInput" required v-on:input="sendSearchQueryEvent" :placeholder="'Search in '+ amounts._total +' technology components ...'  ">
      </b-form-input>
      <b-input-group-append>
        <b-button title="Search" v-on:click="sendSearchQueryEvent" variant="secondary">
          <v-icon name="search" />
        </b-button>
        <b-button title="Clear Filters" variant="secondary" :disabled="!filterOn" @click="clearFilters">
          <v-icon label="reset filters">
  ...
  <b-card-group class="mb-2">
    <b-card overlay v-for="type in types" :key="type" tag="article" class="card-lifecycle">
      <div class="card-body text-left">
        <b-img fluid class="image-lc" :src="images(type)" />
        <b-button :variant="'outline-'+btnVariant(type)" class="phase" @click="setPhase(type)" :pressed="phaseModel[type]">
          {{type | capitalize}}
          <b-badge pill size="lg" :variant="btnVariant(type)">
            {{amountsForType[type]}}
          </b-badge>
        </b-button>

        <b-button-group vertical>
          <span class="bgroup" v-for="item in itemsForType[type]" :key="item">
            <life-cycle :id="item+'selector'" ispopup :status="item" v-on:selected="setLcQuery(item)" :isPressed.sync="lcModel[item]">
            </life-cycle>
  ...
  <b-card style="card control-left" no-body>
    ...
    <div style="card-body father">
      <sunburst-wrap ref="sunburst" :tree="tree" class="sunburst" v-on:nodeClicked="setUseCaseQuery">
      </sunburst-wrap>
    </div>
  </b-card>
</div>
...
export default {
  ...
  data() {
    return {
      searchInput: '',
      lcModel: this.$store.getters.lifeCycle.reduce((acc, i) => ({ ...acc,
        [i.name]: false
      }), {}),
  ...
  created() {
    ...
    this.phaseModel = this.types.reduce((acc, i) => ({ ...acc,
      [i]: false
    }), {})
  ...
  computed: {
    ...
    amountsForType() {
      return this.types.reduce(
        (acc, t) => ({ ...acc,
          [t]: this.itemsForType[t].reduce((sum, lc) => (sum + this.amounts[lc]), 0)
        }), {})
    }
  },
  filters: {
