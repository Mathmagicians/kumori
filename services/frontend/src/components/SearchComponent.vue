  ...
  <div>
    <b-card no-body class="mb-2">
      <b-input-group>
        <b-form-input
          ...
          :placeholder="
            'Search in ' + amounts._total + ' technology components ...'
          "
        >
        </b-form-input>
    ...
    <b-card style="card control-left" no-body>
      <div class="card-header">
        Search in Usecases
        <strong v-if="query">{{ query.tx | stringify }}</strong>
      </div>
...
export default {
  ...
  data() {
    return {
      searchInput: "",
      lcModel: this.$store.getters.lifeCycle.reduce(
        (acc, i) => ({ ...acc, [i.name]: false }),
        {}
      ),
  ...
  created() {
    ...
    this.phaseModel = this.types.reduce(
      (acc, i) => ({ ...acc, [i]: false }),
      {}
    );
  ...
  computed: {
    filterOn() {
      return (
        this.searchInput !== "" ||
        Object.values(this.lcModel).some(lcValue => lcValue) ||
        this.query.tx.length !== 0
      );
    },
    amountsForType() {
      return this.types.reduce(
        (acc, t) => ({
          ...acc,
          [t]: this.itemsForType[t].reduce(
            (sum, lc) => sum + this.amounts[lc],
            0
          )
        }),
        {}
  ...
  methods: {
    ...
    setLcQuery(item, newValue = !this.lcModel[item]) {
      ...
      this.query.lc = Object.keys(this.lcModel).filter(
        item => this.lcModel[item]
      );
      this.updateRoute();
    },
