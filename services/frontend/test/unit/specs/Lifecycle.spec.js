...
localVue.use(Vuex)
const factory = (store, propsData) => mount(LifeCycle, {
  localVue,
  store,
  propsData
})
  ...
  it('Button color corresponds to phase of status property', () => {
    ...
    console.log(button.text())
		/*
			...
			const experimentalColor = factory(store, {
			  status: 'experimental'
			}).find('button').element.style.color
			const defaultColor = factory(store, {
			  status: 'default'
			}).find('button').element.style.color
			const missingColor = factory(store, {
			  status: 'julemand'
			}).find('button').element.style.color
			*/
    // TOOO FIXME
