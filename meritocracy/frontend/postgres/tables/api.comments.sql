...
(
    ...
    CONSTRAINT component FOREIGN KEY (component)
        REFERENCES api.components (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
);

DROP TRIGGER IF EXISTS comments_insert ON api.comments;
CREATE TRIGGER comments_insert BEFORE INSERT
  ON api.comments FOR EACH ROW EXECUTE PROCEDURE row_inserted ();

DROP TRIGGER IF EXISTS comments_update ON api.comments;
CREATE TRIGGER comments_update BEFORE UPDATE
  ON api.comments FOR EACH ROW EXECUTE PROCEDURE row_updated ();

GRANT SELECT (id, comment, component, modified, deleted) ON api.comments TO web_anon;
